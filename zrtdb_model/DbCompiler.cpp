...
fs::path DbCompiler::rustPath() const { return home() / "header" / "rust"; }
...
bool DbCompiler::compileAppFile(const fs::path& appDatFile, std::string& outAppNameUpper)
{
    ...
    try {
        ...
        generateAppRust(outAppNameUpper, app);
...
static std::string rust_type_for(char t, int bytes)
{
    switch (t) {
    case 'I':
        if (bytes == 4)
...
void DbCompiler::generateAppHeader(const std::string& appUpper, const app_strc_dat& app)
{
    ...
    os << "#define ZRTDB_LAYOUT_FINGERPRINT \"" << app.layout_fingerprint << "\"\n\n";
...
void DbCompiler::generateAppRust(const std::string& appUpper, const app_strc_dat& app)
{
    fs::create_directories(rustPath());

    const std::string appLower = sanitize_ident(appUpper, true);
    const std::string appUpperSan = sanitize_ident(appUpper, false);

    auto outFile = rustPath() / (appUpper + ".rs");
    std::ofstream os(outFile);
    if (!os)
        throw std::runtime_error("Cannot write Rust bindings: " + outFile.string());
    ...
    os << "// AUTO-GENERATED by zrtdb_model. DO NOT EDIT.\n";
    os << "// APP: " << appUpper << "\n\n";
    os << "use core::ffi::{c_char, c_int};\n";
    os << "use core::ptr::{addr_of, addr_of_mut, read_unaligned, write_unaligned};\n\n";
    ...
    for (const auto& db : dbs) {
        ...
        for (int r = 0; r < db.record_count; ++r) {
            ...
            os << "pub const ZRTDB_" << dbUpperSan << "_MX_" << recSan << ": usize = " << mx << ";\n";
            os << "pub const ZRTDB_" << dbUpperSan << "_CAP_" << recSan << ": usize = " << cap << ";\n";
        }
        ...
        for (int p = 0; p < db.partition_count; ++p) {
            ...
            if (partUpper.empty())
                continue;
            const std::string partSan = sanitize_ident(partUpper, false);
            os << "pub const ZRTDB_PART_" << dbUpperSan << "_" << partSan << ": &[u8] = b\"" << partUpper << "/" << dbUpper << "\\0\";\n";
        }
    }
    os << "\n";
    ...
    for (const auto& db : dbs) {
        ...
        for (int p = 0; p < db.partition_count; ++p) {
            ...
            for (int f = startFld; f < endFld; ++f) {
                ...
                const char tc = db.field_type[f];
                const int bytes = db.field_bytes[f];
                const std::string rtype = rust_type_for(tc, bytes);

                if (vt == 'A') {
                    os << "    pub _pad" << padCount++ << ": [u8; " << bytes << "],\n";
                    continue;
                }

                if (db.field_record_1based[f] > 0) {
                    const int recIdx = db.field_record_1based[f] - 1;
                    int cap = db.record_max_dim[recIdx];
                    if ((size_t)recIdx < db.record_physical_dim.size() && db.record_physical_dim[(size_t)recIdx] > 0)
                        cap = db.record_physical_dim[(size_t)recIdx];

                    if (tc == 'S' || tc == 'C') {
                        os << "    pub " << fldSan << ": [[" << rtype << "; " << bytes << "]; " << cap << "],\n";
                    } else {
                        os << "    pub " << fldSan << ": [" << rtype << "; " << cap << "],\n";
                    }
                } else {
                    if (tc == 'S' || tc == 'C') {
                        os << "    pub " << fldSan << ": [" << rtype << "; " << bytes << "],\n";
                    } else {
                        os << "    pub " << fldSan << ": " << rtype << ",\n";
                    }
                }
            ...
            for (int f = startFld; f < endFld; ++f) {
                ...
                if (db.field_record_1based[f] > 0) {
                    ...
                    if (tc == 'S' || tc == 'C') {
                        os << "    pub fn get_" << fldSan << "(&self, idx: usize) -> Result<[" << rtype << "; " << bytes << "], ZrtdbAccessError> {\n";
                        os << "        if idx >= " << cap << " { return Err(ZrtdbAccessError::OutOfRange); }\n";
                        os << "        unsafe { Ok(read_unaligned((addr_of!(self." << fldSan << ") as *const [" << rtype << "; " << bytes << "]).add(idx))) }\n";
                        os << "    }\n";
                        os << "    pub fn set_" << fldSan << "(&mut self, idx: usize, v: [" << rtype << "; " << bytes << "]) -> Result<(), ZrtdbAccessError> {\n";
                        os << "        if idx >= " << cap << " { return Err(ZrtdbAccessError::OutOfRange); }\n";
                        os << "        unsafe { write_unaligned((addr_of_mut!(self." << fldSan << ") as *mut [" << rtype << "; " << bytes << "]).add(idx), v); }\n";
                        os << "        Ok(())\n";
                        os << "    }\n";
                    } else {
                        os << "    pub fn get_" << fldSan << "(&self, idx: usize) -> Result<" << rtype << ", ZrtdbAccessError> {\n";
                        os << "        if idx >= " << cap << " { return Err(ZrtdbAccessError::OutOfRange); }\n";
                        os << "        unsafe { Ok(read_unaligned((addr_of!(self." << fldSan << ") as *const " << rtype << ").add(idx))) }\n";
                        os << "    }\n";
                        os << "    pub fn set_" << fldSan << "(&mut self, idx: usize, v: " << rtype << ") -> Result<(), ZrtdbAccessError> {\n";
                        os << "        if idx >= " << cap << " { return Err(ZrtdbAccessError::OutOfRange); }\n";
                        os << "        unsafe { write_unaligned((addr_of_mut!(self." << fldSan << ") as *mut " << rtype << ").add(idx), v); }\n";
                        os << "        Ok(())\n";
                        os << "    }\n";
                    }
                } else {
                    if (tc == 'S' || tc == 'C') {
                        os << "    pub fn get_" << fldSan << "(&self) -> [" << rtype << "; " << bytes << "] { unsafe { read_unaligned(addr_of!(self." << fldSan << ")) } }\n";
                        os << "    pub fn set_" << fldSan << "(&mut self, v: [" << rtype << "; " << bytes << "]) { unsafe { write_unaligned(addr_of_mut!(self." << fldSan << "), v); } }\n";
                    } else {
                        os << "    pub fn get_" << fldSan << "(&self) -> " << rtype << " { unsafe { read_unaligned(addr_of!(self." << fldSan << ")) } }\n";
                        os << "    pub fn set_" << fldSan << "(&mut self, v: " << rtype << ") { unsafe { write_unaligned(addr_of_mut!(self." << fldSan << "), v); } }\n";
                    }
                }
            }
            os << "}\n";
            os << "pub const ZRTDB_SIZE_" << dbUpperSan << "_" << partUpperSan << ": usize = core::mem::size_of::<" << typeName << ">();\n\n";
        }
    }

    os << "#[derive(Debug)]\n";
    os << "pub enum ZrtdbAccessError { NullPtr, OutOfRange }\n\n";
    os << "pub struct ZrtdbRo<T> { ptr: *const T }\n";
    os << "pub struct ZrtdbRw<T> { ptr: *mut T }\n";
    os << "impl<T> ZrtdbRo<T> {\n";
    os << "    pub unsafe fn new(ptr: *const T) -> Result<Self, ZrtdbAccessError> { if ptr.is_null() { Err(ZrtdbAccessError::NullPtr) } else { Ok(Self { ptr }) } }\n";
    os << "    pub fn as_ptr(&self) -> *const T { self.ptr }\n";
    os << "}\n";
    os << "impl<T> ZrtdbRw<T> {\n";
    os << "    pub unsafe fn new(ptr: *mut T) -> Result<Self, ZrtdbAccessError> { if ptr.is_null() { Err(ZrtdbAccessError::NullPtr) } else { Ok(Self { ptr }) } }\n";
    os << "    pub fn as_ptr(&self) -> *const T { self.ptr as *const T }\n";
    os << "    pub fn as_mut_ptr(&mut self) -> *mut T { self.ptr }\n";
    os << "}\n\n";

    os << "#[repr(C)]\n";
    os << "pub struct zrtdb_app_" << appLower << "_ctx_t {\n";
    for (const auto& db : dbs) {
        const std::string dbUpper = upper(db.db_id);
        const std::string dbLower = sanitize_ident(dbUpper, true);
        const std::string dbUpperSan = sanitize_ident(dbUpper, false);
        ...
        for (int p = 0; p < db.partition_count; ++p) {
            ...
            const std::string typeName = "zrtdb_" + dbLower + "_" + partLower + "_t";
            const std::string member = dbUpperSan + "_" + partUpperSan;
            os << "    pub " << member << ": *mut " << typeName << ",\n";
        }
    }
    os << "}\n\n";

    os << "impl zrtdb_app_" << appLower << "_ctx_t {\n";
    for (const auto& db : dbs) {
        const std::string dbUpper = upper(db.db_id);
        const std::string dbLower = sanitize_ident(dbUpper, true);
        const std::string dbUpperSan = sanitize_ident(dbUpper, false);
        ...
        for (int p = 0; p < db.partition_count; ++p) {
            ...
            const std::string typeName = "zrtdb_" + dbLower + "_" + partLower + "_t";
            const std::string member = dbUpperSan + "_" + partUpperSan;
            os << "    pub unsafe fn " << partLower << "_ro(&self) -> Result<ZrtdbRo<" << typeName << ">, ZrtdbAccessError> { ZrtdbRo::new(self." << member << " as *const " << typeName << ") }\n";
            os << "    pub unsafe fn " << partLower << "_rw(&mut self) -> Result<ZrtdbRw<" << typeName << ">, ZrtdbAccessError> { ZrtdbRw::new(self." << member << ") }\n";
        }
    }
    os << "}\n\n";

    os << "pub unsafe fn zrtdb_app_" << appLower << "_init(ctx: *mut zrtdb_app_" << appLower << "_ctx_t) -> c_int {\n";
    os << "    if ctx.is_null() {\n";
    os << "        return -1;\n";
    os << "    }\n";
    os << "    if RegisterApp_(ZRTDB_APP_NAME_" << appUpperSan << ".as_ptr() as *const c_char) < 0 {\n";
    os << "        return -1;\n";
    os << "    }\n";
    ...
    for (const auto& db : dbs) {
        ...
        for (int p = 0; p < db.partition_count; ++p) {
            ...
            os << "    if MapMemory_(ZRTDB_PART_" << dbUpperSan << "_" << partUpperSan
               << ".as_ptr() as *const c_char, &mut (*ctx)." << member << " as *mut _ as *mut *mut c_char) < 0 {\n";
            os << "        return -1;\n";
            os << "    }\n";
        }
    }
    ...
    os.close();
    std::cout << "[RUST] " << appUpper << " -> " << outFile << std::endl;
}
